...
const private = {
  MenuArrayToObject(menuArray) {
    return Object.fromEntries(menuArray.map(arr => [`${arr[0]} (${arr[2]})`, `$${arr[1]}`]));
  },
  ...
  generateUserCart(menuArray, OrderArray, prevUserCart) {
    ...
    const cartObj = OrderArray.reduce((orderObj, orderItem) => {
      const matchingMenuItem = menuItems.find(itemName => itemName.includes(orderItem));
      if (!matchingMenuItem) {
        throw helpers.errorWrapper(
          `Order, ${orderItem}, is not available in our menu. Order wasn't saved`,
          400
        );
      }

      const price = helpers.convertDollarToFloat(menuObject[matchingMenuItem]);
      const count = orderObj[matchingMenuItem]
        ? helpers.convertDollarToFloat(orderObj[matchingMenuItem]) / price
        : 1;

      orderObj[matchingMenuItem] = orderObj[matchingMenuItem]
        ? `$${(helpers.convertDollarToFloat(orderObj[matchingMenuItem]) + price).toFixed(2)} (x${
            count + 1
          })`
        : `$${price} (x1)`;
...
const orderHandler = {
  async order(data) {
    ...
    if (authenticatedEmail?.statusCode) return authenticatedEmail;
