...
const tokensRouteMethodObj = {
  ...
  async post(data) {
    ...
    if (fileData?.hasToken) {
      ...
      try {
        const newToken = await auth.verifyToken(tokenHashId, email);

        const tokenToRead = newToken?.Id || tokenHashId;
  ...
  async tokens(data) {
    ...
    try {
      if (method === 'post') {
        ...
        newAccessToken = await auth.verifyToken(reqInputObj.token, emailToCheckTokenAgainst);
