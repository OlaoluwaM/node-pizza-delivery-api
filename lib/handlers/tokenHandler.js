  ...
  async post(data) {
    ...
    if (fileData?.hasToken) {
      ...
      if (authenticationResult.type === 'success' && authenticationResult.isValid) {
        if (authenticationResult?.newToken) {
          return helpers.generateResponseObj(200, authenticationResult.newToken);
        }

        return helpers.generateResponseObj(
          200,
          authenticationResult?.msg || 'User already has a valid token'
        );
      } else {
