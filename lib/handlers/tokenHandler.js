...
const tokensRouteMethodObj = {
  ...
  async post(data) {
    ...
    // try {
    //   const { type, data: tokenData } = _data.read('tokens', users);
    //   if (type === 'error') throw tokenData;
    // } catch (error) {
    //   if (helpers.validateType(error, 'string')&&  error.search(//) > -1)
    // }
    ...
    if (fileData?.hasToken) {
      ...
      if (authenticationResult.type === 'success' && authenticationResult.isValid) {
        ...
        if (errObj.data.search(/no such file/i) > -1) {
          delete fileData?.hasToken;
        } else if (errObj) throw errObj.data;
      }
    }
