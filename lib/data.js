...
const dataLib = {
  async create(dir, file, dataToWriteToFile, update = false) {
    ...
    try {
      ...
      let errorMsg = `There was an error creating the file: '${file}'. ${err}`;

      if (update) {
        errorMsg = `There was an error updating the file: '${file}'. ${err}`;
      }
  ...
  async doesFileExist(dir, file) {
    ...
    try {
      ...
      throw new CustomError('Cannot access file', 500);
    }
  },
};
