...
const dataLib = {
  async create(dir, file, dataToWriteToFile, update = false) {
    ...
    try {
      ...
      if (!update) console.log(`Success, ${file} file created`);

      return;
    } catch {
      if (update) {
        return `There was an error updating the '${file}' in the '${dir}' directory`;
      }
      const errorMsg = `There was an error creating the '${file}' file in the '${dir}' directory. It may already exist`;
  ...
  async read(dir, file) {
    ...
    try {
      ...
      const errorMsg = `There was an error reading from file '${file}', in '${dir}' directory, the file may not exist. ${error}`;
      console.error(errorMsg);
