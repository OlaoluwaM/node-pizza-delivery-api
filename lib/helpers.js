...
function generateResponseObj(statusCode = 200, data = {}) {
  return { statusCode, returnedData: JSON.stringify({ response: data }) };
}
...
function generateTokenExpiration(tokenType) {
  if (tokenType === 'refresh') {
    return Date.now() + 1000 * 3600 * 50.7;
  } else return Date.now() + 1000 * 1800;
}
